...
CoordSystemSphere::CoordSystemSphere(const picojson::object& config) :
    ...
    conf_radius(0),
    my_position((2.0 * M_PI * static_cast<double>(Context::get_rnd_32()) /
                 static_cast<double>(UINT32_MAX)) - M_PI * 1.0,
                (1.0 * M_PI * static_cast<double>(Context::get_rnd_32()) /
                 static_cast<double>(UINT32_MAX)) - M_PI * 0.5) {
...
double CoordSystemSphere::get_distance(const Coordinate& p1, const Coordinate& p2) {
  ...
  return conf_radius * 2 * std::asin(std::sqrt(std::pow(std::sin(avr_y), 2) + std::cos(p1.y) *
                                     std::cos(p2.y) * std::pow(std::sin(avr_x), 2)));
}
