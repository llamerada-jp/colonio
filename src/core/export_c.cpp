...
class ColonioC : public colonio::Colonio {
 public:
  ...
  ColonioC() :
      colonio(nullptr),
      cb_on_require_invoke(nullptr),
      cb_on_output_log(nullptr),
      cb_on_debug_event(nullptr) {
  }
 ...
 protected:
  ...
  void on_output_log(colonio::LogLevel::Type level, const std::string& message) override {
    if (cb_on_output_log != nullptr) {
      cb_on_output_log(colonio, static_cast<COLONIO_LOG_LEVEL>(level),
...
void colonio_connect(colonio_t* colonio, const char* url, const char* token,
                  void (*on_success)(colonio_t*),
                  void (*on_failure)(colonio_t*)) {
  colonio_export_c::ColonioC* impl = reinterpret_cast<colonio_export_c::ColonioC*>(colonio->impl);
  impl->connect(url, token,
                [on_success, colonio](colonio::Colonio&) { on_success(colonio); },
...
void colonio_set_on_output_log(colonio_t* colonio, void (*func)(colonio_t*, COLONIO_LOG_LEVEL, const char*, unsigned int)) {
  colonio_export_c::ColonioC* impl = reinterpret_cast<colonio_export_c::ColonioC*>(colonio->impl);
  impl->cb_on_output_log = func;
}

void colonio_set_on_debug_event(colonio_t* colonio, void (*func)(colonio_t*, COLONIO_DEBUG_EVENT, const char*, unsigned int)) {
  colonio_export_c::ColonioC* impl = reinterpret_cast<colonio_export_c::ColonioC*>(colonio->impl);
...
void colonio_map_get(colonio_map_t* map, const colonio_value_t* key, void* ptr,
...
void colonio_map_set(colonio_map_t* map, const colonio_value_t* key, const colonio_value_t* value, void* ptr,
...
void colonio_pubsub2d_publish(colonio_pubsub2d_t* pubsub2d, const char* name, unsigned int name_siz,
                           double x, double y, double r, const colonio_value_t* value, void* ptr,
                           void (*on_success)(colonio_pubsub2d_t* pubsub2d, void* ptr),
                           void (*on_failure)(colonio_pubsub2d_t* pubsub2d, void* ptr,
                                              COLONIO_PUBSUB2D_FAILURE_REASON reason)) {
  colonio::PubSub2D* impl = reinterpret_cast<colonio::PubSub2D*>(pubsub2d->impl);
  colonio::Value cpp_value;
  convert_value_c_to_cpp(&cpp_value, value);
  impl->publish(std::string(name, name_siz), x, y, r, cpp_value,
                [pubsub2d, ptr, on_success]() {
                  on_success(pubsub2d, ptr);
                },
...
void colonio_pubsub2d_on(colonio_pubsub2d_t* pubsub2d, const char* name, unsigned int name_siz, void* ptr,
                      void (*subscriber)(colonio_pubsub2d_t* pubsub2d, void* ptr, const colonio_value_t* value)) {
  colonio::PubSub2D* impl = reinterpret_cast<colonio::PubSub2D*>(pubsub2d->impl);
  impl->on(std::string(name, name_siz),
           [pubsub2d, ptr, subscriber](const colonio::Value& value) {
             colonio_value_t c_value;
             convert_value_cpp_to_c(&c_value, &value);
