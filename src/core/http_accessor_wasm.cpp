...
extern "C" {
  ...
  extern void http_accessor_post(COLONIO_PTR_T http_accessor, COLONIO_PTR_T delegate,
                                 COLONIO_PTR_T url, COLONIO_PTR_T content_type,
                                 COLONIO_PTR_T payload_ptr, int payload_siz);
  EMSCRIPTEN_KEEPALIVE void http_accessor_on_response(COLONIO_PTR_T this_ptr, COLONIO_PTR_T delegate_ptr,
                                                      int code_ptr, COLONIO_PTR_T content, int content_siz);
  EMSCRIPTEN_KEEPALIVE void http_accessor_on_error(COLONIO_PTR_T this_ptr, COLONIO_PTR_T delegate_ptr,
                                                   COLONIO_PTR_T message_ptr, int message_siz,
                                                   COLONIO_PTR_T payload_ptr, COLONIO_PTR_T payload_siz);
}

void http_accessor_on_response(COLONIO_PTR_T this_ptr, COLONIO_PTR_T delegate_ptr,
                               int code, COLONIO_PTR_T content_ptr, int content_siz) {
...
void http_accessor_on_error(COLONIO_PTR_T this_ptr, COLONIO_PTR_T delegate_ptr,
                            COLONIO_PTR_T message_ptr, int message_siz,
                            COLONIO_PTR_T payload_ptr, COLONIO_PTR_T payload_siz) {
  colonio::HttpAccessorWasm& THIS = *reinterpret_cast<colonio::HttpAccessorWasm*>(this_ptr);
  colonio::HttpAccessorDelegate& delegate = *reinterpret_cast<colonio::HttpAccessorDelegate*>(delegate_ptr);
...
void HttpAccessorWasm::post(const std::string& url, const std::string& payload,
                            const std::string& content_type, HttpAccessorDelegate* delegate) {
  assert(delegate != nullptr);

  http_accessor_post(reinterpret_cast<COLONIO_PTR_T>(this),
                     reinterpret_cast<COLONIO_PTR_T>(delegate),
                     reinterpret_cast<COLONIO_PTR_T>(url.c_str()),
                     reinterpret_cast<COLONIO_PTR_T>(content_type.c_str()),
                     reinterpret_cast<COLONIO_PTR_T>(payload.c_str()),
                     payload.size());
}
}  // namespace colonio
