...
func newResponseWriter(logger *slog.Logger, transferer *transferer.Transferer, packet *networkTypes.Packet) ResponseWriter {
...
func (r *messagingResponseHandler) OnResponse(packet *networkTypes.Packet) {
...
func (m *Messaging) Post(dstNodeID *types.NodeID, name string, val []byte, opt *Options) ([]byte, error) {
...
func (m *Messaging) recvMessage(packet *networkTypes.Packet) {
	...
	opt := &Options{
		AcceptNearby:   (packet.Mode & networkTypes.PacketModeExplicit) == 0,
		IgnoreResponse: (packet.Mode & networkTypes.PacketModeOneWay) != 0,
	}
