...
func TestRouting2D_updateNodeConnections(t *testing.T) {
	...
	tests := []struct {
		...
		expect      map[types.NodeID]*routeInfo2D
	}{
		{
			routeInfos: map[types.NodeID]*routeInfo2D{},
			connections: map[types.NodeID]struct{}{
...
func TestRouting2D_setupRoutingPacket(t *testing.T) {
	...
	r.routeInfos = map[types.NodeID]*routeInfo2D{
		*nodeIDs[0]: {
	...
	for _, nodeID := range []*types.NodeID{nodeIDs[0], nodeIDs[1], nodeIDs[4]} {
...
func TestRouting2D_neighborNodeIDChanged(t *testing.T) {
	...
	pointsMap := make(map[delaunay.Point]*types.NodeID)
	for i := 0; i < 100; i++ {
	...
	r.updateLocalPosition(positions[0])
	r.routeInfos = make(map[types.NodeID]*routeInfo2D)
	for i := 1; i < 100; i++ {
	...
	expect := make(map[types.NodeID]struct{})
