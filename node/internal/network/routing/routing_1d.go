...
var levelRanges []*types.NodeID = []*types.NodeID{
	types.NewNormalNodeID(0x0001FFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 0
	types.NewNormalNodeID(0x0007FFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 1
	types.NewNormalNodeID(0x001FFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 2
	types.NewNormalNodeID(0x007FFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 3
	types.NewNormalNodeID(0x01FFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 4
	types.NewNormalNodeID(0x07FFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 5
	types.NewNormalNodeID(0x1FFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 6
	types.NewNormalNodeID(0x7FFFFFFFFFFFFFFF, 0xFFFFFFFFFFFFFFFF), // 7
}
...
type routing1DConfig struct {
	...
	reconcileNextNodes func(nextNodeIDs, disconnectedNodeIDs []*types.NodeID) (bool, error)
}
...
func newRouting1D(config *routing1DConfig) *routing1D {
	return &routing1D{
		config:            config,
		neighborhoodInfos: make(map[types.NodeID]*neighborhoodInfo),
	...
	for nodeID := range connections {
		if _, ok := r.neighborhoodInfos[nodeID]; !ok {
			r.neighborhoodInfos[nodeID] = &neighborhoodInfo{
				...
				secondNeighborhoods: make(map[types.NodeID]struct{}),
			}
			updated = true
		}
	}
...
func (r *routing1D) getNextStep(packet *networkTypes.Packet) *types.NodeID {
	isExplicit := (packet.Mode & networkTypes.PacketModeExplicit) != 0

	if packet.DstNodeID.Equal(&types.NodeLocal) ||
		packet.DstNodeID.Equal(r.config.localNodeID) {
		return &types.NodeLocal
	}

	if packet.DstNodeID.Equal(&types.NodeNeighborhoods) {
		return &types.NodeNeighborhoods
	}
...
func (r *routing1D) countRecvPacket(from *types.NodeID) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
...
func (r *routing1D) recvRoutingPacket(src *types.NodeID, content *proto.Routing) (int, error) {
...
func (r *routing1D) getConnections() (map[types.NodeID]struct{}, map[types.NodeID]struct{}) {
	r.mtx.Lock()
	r.normalizeScore()
	r.mtx.Unlock()

	now := time.Now()
	required := make(map[types.NodeID]struct{})
	keep := make(map[types.NodeID]struct{})
	requiredLevelCount := make(map[int]int)
	candidatesByLevel := make(map[int]map[types.NodeID]int)
	for lv := 0; lv < levelRangesCount; lv++ {
		candidatesByLevel[lv] = make(map[types.NodeID]int)
	}
	...
	for neighborhoodNodeID, neighborhoodInfo := range r.neighborhoodInfos {
		...
		largerNodeIDs := []*types.NodeID{r.config.localNodeID}
		smallerNodeIDs := []*types.NodeID{r.config.localNodeID}
...
// private
func (r *routing1D) calcLevel(nodeID *types.NodeID) int {
...
func (r *routing1D) updateRouteInfos() {
	idMap := make(map[types.NodeID]*types.NodeID)
...
func (r *routing1D) updateNextNodeIDs() {
	...
	knownNodeIDs := make(map[types.NodeID]struct{})
	for nodeID, info := range r.neighborhoodInfos {
	...
	if len(knownNodeIDs) == 1 {
		for nodeID := range knownNodeIDs {
			r.frontwardNextNodeIDs = []*types.NodeID{&nodeID}
			r.backwardNextNodeIDs = []*types.NodeID{&nodeID}
		}
		return
	}

	largerNodeIDs := make([]*types.NodeID, 0)
	smallerNodeIDs := make([]*types.NodeID, 0)
	for nodeID := range knownNodeIDs {
...
func (r *routing1D) updateNextNodeMatched(previousNextNodeIDs []*types.NodeID) {
	...
	var disconnectedNodeIDs []*types.NodeID
	if len(previousNextNodeIDs) != 0 {
		disconnectedNodeIDs = make([]*types.NodeID, 0)
