...
type networkHandlerHelper struct {
	updateNextNodePosition func(map[types.NodeID]*geometry.Coordinate)
}

func (h *networkHandlerHelper) NetworkUpdateNextNodePosition(p map[types.NodeID]*geometry.Coordinate) {
...
func TestNetwork(t *testing.T) {
	...
	positionMaps := make([]map[types.NodeID]*geometry.Coordinate, len(nodeIDs))
	...
	config.Handler = &networkHandlerHelper{
		updateNextNodePosition: func(m map[types.NodeID]*geometry.Coordinate) {
			mtx.Lock()
			defer mtx.Unlock()
			positionMaps[0] = m
		},
	...
	receivedPackets := make([]*networkTypes.Packet, 0)
	transferer.SetRequestHandler[proto.PacketContent_Error](networks[0].GetTransferer(), func(p *networkTypes.Packet) {
		mtx.Lock()
		defer mtx.Unlock()
	...
	for i := 1; i < len(nodeIDs); i++ {
		...
		transferer.SetRequestHandler[proto.PacketContent_Error](networks[i].GetTransferer(), func(p *networkTypes.Packet) {
	...
	expectedPositionMap := make(map[types.NodeID]*geometry.Coordinate)
	...
	// send packet
	networks[1].GetTransferer().RequestOneWay(nodeIDs[0], networkTypes.PacketModeExplicit, &proto.PacketContent{
	...
	networks[2].GetTransferer().RequestOneWay(nodeIDs[0].Add(types.NewNormalNodeID(0, 1)), networkTypes.PacketModeNone, &proto.PacketContent{
		Content: &proto.PacketContent_Error{
			Error: &proto.Error{
				Code:    2,
				Message: "test",
	...
	slices.SortFunc(sortedNodeIDs, func(a, b *types.NodeID) int {
		return a.Compare(b)
	})
	for i, network := range networks {
		nodeID := nodeIDs[i]
