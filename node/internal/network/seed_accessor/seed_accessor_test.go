...
type seedAccessorHandlerHelper struct {
	...
	recvSignalOffer  func(*types.NodeID, *signal.Offer)
	recvSignalAnswer func(*types.NodeID, *signal.Answer)
	recvSignalICE    func(*types.NodeID, *signal.ICE)
}
...
func (h *seedAccessorHandlerHelper) SeedRecvSignalOffer(srcNodeID *types.NodeID, offer *signal.Offer) {
	if h.recvSignalOffer != nil {
		h.recvSignalOffer(srcNodeID, offer)
	}
}

func (h *seedAccessorHandlerHelper) SeedRecvSignalAnswer(srcNodeID *types.NodeID, answer *signal.Answer) {
	if h.recvSignalAnswer != nil {
		h.recvSignalAnswer(srcNodeID, answer)
	}
}

func (h *seedAccessorHandlerHelper) SeedRecvSignalICE(srcNodeID *types.NodeID, ice *signal.ICE) {
...
func TestSeedAccessor_SignalingKind(t *testing.T) {
	...
	dstAccessor := newAccessor(server, nil, &seedAccessorHandlerHelper{
		...
		recvSignalOffer: func(srcNodeID *types.NodeID, offer *signal.Offer) {
			mtx.Lock()
			defer mtx.Unlock()
			receivedCounts[0]++
		...
		recvSignalAnswer: func(srcNodeID *types.NodeID, answer *signal.Answer) {
			mtx.Lock()
			defer mtx.Unlock()
			receivedCounts[1]++
			assert.True(t, srcNodeID.Equal(nodeIDs[0]))
		...
		recvSignalICE: func(srcNodeID *types.NodeID, ice *signal.ICE) {
			mtx.Lock()
			defer mtx.Unlock()
			receivedCounts[2]++
			assert.True(t, srcNodeID.Equal(nodeIDs[0]))
...
func TestSeedAccessor_SignalingTarget(t *testing.T) {
	...
	mtx := sync.Mutex{}
	receivedFrom := make([][]*types.NodeID, len(nodeIDs))
	for i := range nodeIDs {
...
func TestSeedAccessor_ReconcileNextNode(t *testing.T) {
	...
	gateway := &helper.Gateway{
		NodeIDs: []*types.NodeID{nodeIDs[0]},
		GetNodesByRangeF: func(ctx context.Context, backward, frontward *types.NodeID) ([]*types.NodeID, error) {
			...
			return []*types.NodeID{nodeIDs[1], nodeIDs[2]}, nil
		},
	...
	res, err := accessor.ReconcileNextNodes(
		[]*types.NodeID{nodeIDs[1]},
		[]*types.NodeID{nodeIDs[2]},
	)
	require.NoError(t, err)
	assert.False(t, res)
}
