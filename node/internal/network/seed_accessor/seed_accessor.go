...
type Handler interface {
	SeedRecvSignalOffer(*types.NodeID, *signal.Offer)
	SeedRecvSignalAnswer(*types.NodeID, *signal.Answer)
	SeedRecvSignalICE(*types.NodeID, *signal.ICE)
}
...
func (sa *SeedAccessor) Start(ctx context.Context) (*types.NodeID, error) {
	sa.ctx = ctx
	...
	sa.mtx.Lock()
	defer sa.mtx.Unlock()
	sa.localNodeID, err = types.NewNodeIDFromProto(res.Msg.GetNodeId())
	if err != nil {
...
func (sa *SeedAccessor) SendSignalOffer(dstNodeID *types.NodeID, offer *signal.Offer) error {
...
func (sa *SeedAccessor) SendSignalAnswer(dstNodeID *types.NodeID, answer *signal.Answer) error {
	_, err := sa.client.SendSignal(sa.ctx, &connect.Request[proto.SendSignalRequest]{
		Msg: &proto.SendSignalRequest{
			Signal: &proto.Signal{
				DstNodeId: dstNodeID.Proto(),
...
func (sa *SeedAccessor) SendSignalICE(dstNodeID *types.NodeID, ices *signal.ICE) error {
	_, err := sa.client.SendSignal(sa.ctx, &connect.Request[proto.SendSignalRequest]{
		Msg: &proto.SendSignalRequest{
			Signal: &proto.Signal{
				DstNodeId: dstNodeID.Proto(),
...
func (sa *SeedAccessor) ReconcileNextNodes(nextNodeIDs, disconnectedNodeIDs []*types.NodeID) (bool, error) {
	res, err := sa.client.ReconcileNextNodes(sa.ctx, &connect.Request[proto.ReconcileNextNodesRequest]{
		Msg: &proto.ReconcileNextNodesRequest{
			NextNodeIds:         types.ConvertNodeIDsToProto(nextNodeIDs),
			DisconnectedNodeIds: types.ConvertNodeIDsToProto(disconnectedNodeIDs),
		},
	})
...
func (sa *SeedAccessor) poll() error {
	...
	for res.Receive() {
		...
		for _, s := range msg.GetSignals() {
			from, err := types.NewNodeIDFromProto(s.GetSrcNodeId())
			if err != nil {
