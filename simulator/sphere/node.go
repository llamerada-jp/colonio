...
func RunNode(ctx context.Context, seedURL string, writer *datastore.Mongodb) error {
	...
	col := colonio.NewColonio(
		...
		colonio.WithObservationHandlers(&colonio.ObservationHandlers{
	...
	for {
		...
		case <-timer.C:
			...
			record.X = position.x
			record.Y = position.y
			writer.Push(col.GetLocalNodeID(), record)
		}
	}
}
