...
func nodeIDtoY(nodeID *types.NodeID) float64 {
	raw0, _ := nodeID.Raw()
...
func (f *RenderFramework) gatherSectorInfo() []sectorInfo {
	...
	for nodeIDstr, n := range f.AliveNodes {
		for _, sectorInfo := range n.Record.GetRecord().SectorInfos {
			...
			var nodeID, head, tail *types.NodeID
			nodeID, _ = types.NewNodeIDFromString(nodeIDstr)
			head, _ = types.NewNodeIDFromString(sectorInfo.Head)
			if sectorInfo.Tail != "" {
				tail, _ = types.NewNodeIDFromString(sectorInfo.Tail)
			}
			sectorRecords[sectorInfo.SectorID] = append(sectorRecords[sectorInfo.SectorID], sectorRecord{
				sectorNo: kvsTypes.SectorNo(sectorInfo.SectorNo),
				nodeID:   nodeID,
				head:     head,
				tail:     tail,
			})
