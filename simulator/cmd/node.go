...
import (
	...
	"github.com/llamerada-jp/colonio/simulator/stories/sphere"
...
func run(ctx context.Context, writer *datastore.Mongodb) error {
	col := colonio.NewColonio(colonio.WithContext(ctx), colonio.WithInsecure())
	if err := col.Connect(nodeConfig.seedURL, nil); err != nil {
		return err
	}
	...
	case "sphere":
		return sphere.Run(ctx, col, writer)
	default:
