...
func TestRequestAndResponse(t *testing.T) {
	...
	localNID := shared.NewRandomNodeID()
	dstNodeID := shared.NewRandomNodeID()
	...
	assert.True(t, localNID.Equal(packets[0].SrcNodeID))
	...
	assert.True(t, localNID.Equal(packets[1].SrcNodeID))
	...
	assert.True(t, localNID.Equal(responsePacket.DstNodeID))
	assert.True(t, localNID.Equal(responsePacket.SrcNodeID))
...
func TestRequestOneWay(t *testing.T) {
	...
	assert.True(t, localNID.Equal(packet.SrcNodeID))
