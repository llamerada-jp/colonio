...
func (rs *routingSeed) updateNodeConnections(connections map[shared.NodeID]struct{}) bool {
	...
	if _, ok := rs.seedRouteInfos[*rs.nextForSeed]; !ok {
		rs.nextForSeed = nextCandidate
		return true
	}
...
func (rs *routingSeed) setupRoutingPacket(content *proto.Routing) {
	...
	if rs.nextForSeed.Equal(&shared.NodeIDThis) {
		content.SeedDistance = 1
	} else if info, ok := rs.seedRouteInfos[*rs.nextForSeed]; ok {
...
func (rs *routingSeed) updateNextForSeed() bool {
	prevNext := rs.nextForSeed
	prevDistance := rs.distanceFromSeed

	if _, ok := rs.seedRouteInfos[*rs.nextForSeed]; !ok {
		rs.nextForSeed = &shared.NodeIDNone
	...
	for nodeID, info := range rs.seedRouteInfos {
		...
		if info.distance < rs.distanceFromSeed {
			rs.nextForSeed = &nodeID
			rs.distanceFromSeed = info.distance
		}
	}

	if !prevNext.Equal(rs.nextForSeed) || prevDistance != rs.distanceFromSeed {
		return true
	}
	return false
}
...
func (rs *routingSeed) reviewConnectionToSeed() {
	...
	info := rs.seedRouteInfos[*rs.nextForSeed]
