...
func newWebRTCLinkNative(c *webRTCLinkConfig, eventHandler *webRTCLinkEventHandler) (webRTCLink, error) {
	...
	config, ok := configurationMap[configID]
	if !ok {
		return nil, fmt.Errorf("failed to get webrtc config")
	}
...
func (w *webRTCLinkNative) setDataChannelHandler(dc *webrtc.DataChannel) {
	...
	w.dataChannel.OnError(func(err error) {
		w.mtx.RLock()
		defer w.mtx.RUnlock()
		if !w.active {
			return
...
func (w *webRTCLinkNative) disconnect() error {
	...
	defer func() {
		...
		if call {
			w.eventHandler.changeLinkState(a, o)
		}
	}()
