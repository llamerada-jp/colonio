...
func (k *KVS) sendSettingMessage() {
	for sectorNo, ms := range k.memberStates {
		...
		case memberStateCreating, memberStateAppending, memberStateConfiguredConsensus:
			...
			k.infrastructure.sendSectorManageMember(&SectorManageMemberParam{
				dstNodeID: ms.nodeID,
...
func (k *KVS) sectorManageMemberResponse(srcNodeID *shared.NodeID, sectorID config.SectorID, sectorNo config.SectorNo) {
	k.mtx.Lock()
	defer k.mtx.Unlock()
