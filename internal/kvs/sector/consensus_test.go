...
type consensusHandlerHelper struct {
	...
	consensusAppendNodeF    func(sectorNo config.SectorNo, nodeID *shared.NodeID)
	consensusRemoveNodeF    func(sectorNo config.SectorNo)
...
var _ consensusHandler = &consensusHandlerHelper{}
...
func (h *consensusHandlerHelper) consensusAppendNode(sectorNo config.SectorNo, nodeID *shared.NodeID) {
	h.t.Helper()
	require.NotNil(h.t, h.consensusAppendNodeF)
	h.consensusAppendNodeF(sectorNo, nodeID)
}

func (h *consensusHandlerHelper) consensusRemoveNode(sectorNo config.SectorNo) {
	h.t.Helper()
...
type consensusInfrastructureHelper struct {
	t                     *testing.T
	sendConsensusMessageF func(dstNodeID *shared.NodeID, message *proto.ConsensusMessage)
}

var _ ConsensusInfrastructure = &consensusInfrastructureHelper{}

func (h *consensusInfrastructureHelper) sendConsensusMessage(dstNodeID *shared.NodeID, message *proto.ConsensusMessage) {
	h.t.Helper()
...
func TestConsensus(t *testing.T) {
	...
	sectorNos := testUtil.UniqueNumbersU[config.SectorNo](sectorCount)
	...
	for i := 0; i < sectorCount; i++ {
		...
		infrastructures[i] = &consensusInfrastructureHelper{
			t: t,
			sendConsensusMessageF: func(dstNodeID *shared.NodeID, message *proto.ConsensusMessage) {
				assert.Equal(t, message.SectorId, MustMarshalSectorID(sectorID))
				i := sectorNoMap[config.SectorNo(message.SectorNo)]
	...
	t.Run("create the initial raft cluster with 3 nodes", func(tt *testing.T) {
		...
		for i := 0; i < initialMemberCount; i++ {
			consensuses[i] = newConsensus(&consensusConfig{
	...
	t.Run("add a new node", func(tt *testing.T) {
		...
		consensuses[additionalNodeIdx] = newConsensus(&consensusConfig{
			logger:         testUtil.Logger(t),
	...
	t.Run("send proposal", func(tt *testing.T) {
		...
		proposalStrs := []string{}
		for _, p := range proposals {
			proposalStrs = append(proposalStrs, p.prop.String())
		}
		for i := 1; i < sectorCount; i++ {
