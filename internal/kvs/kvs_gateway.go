...
type KvsUsecase interface {
	...
	SectorConfigure(param *SectorConfigureParam) error
	SectorConfigureResponse(srcNodeID *shared.NodeID, sectorID config.SectorID, sectorNo config.SectorNo)
...
func NewKvsGateway(l *slog.Logger, t *transferer.Transferer, u KvsUsecase) *KvsGateway {
	...
	transferer.SetRequestHandler[proto.PacketContent_SectorConfig](t, g.recvSectorConfig)
	transferer.SetRequestHandler[proto.PacketContent_SectorConfigResponse](t, g.recvSectorConfigResponse)
...
func (g *KvsGateway) recvSectorConfig(packet *shared.Packet) {
	content := packet.Content.GetSectorConfig()
	...
	if err := g.usecase.SectorConfigure(&SectorConfigureParam{
		command: command,
	...
	g.transferer.RequestOneWay(
		...
		&proto.PacketContent{
			Content: &proto.PacketContent_SectorConfigResponse{
				SectorConfigResponse: &proto.SectorConfigResponse{
...
func (g *KvsGateway) recvSectorConfigResponse(packet *shared.Packet) {
	content := packet.Content.GetSectorConfigResponse()
	sectorID, err := sector.UnmarshalSectorID(content.SectorId)
	if err != nil {
	...
	g.usecase.SectorConfigureResponse(packet.SrcNodeID, sectorID, sectorNo)
}
