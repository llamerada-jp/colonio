...
`spec/kvs/states/26-07-22-19-30-01.996/`（512GB）は 2026-07-25 に削除した。
`-recover` は2回とも失敗しており再開には使えなかったため、再現実験が
必要になった場合は本メモの手順で新規にフレッシュ実行して再現すること
（`spec/**/states/` は `.gitignore` 対象で追跡外）。
