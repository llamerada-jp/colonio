...
- **is_stable ゲートによる修復凍結**: `subRoutine` は is_stable でないと
  ...
  **現状確認 (2026-07-25)**: run 4/11/14 の観測を引き起こしていた根本原因
  （seed セッション喪失による「接続黒穴」ノード）は 2026-07-11 に別件で
  解消済み。run 15（6.8 時間、黒穴 0 体）では is_stable フラップ自体は
  残るが「停滞への寄与は観測されず」（README run 15 節）。ゲートの構造的
  脆弱性は未修正のまま残るが実害は現状観測できておらず、対策の着手は
  保留（詳細は README「is_stable ゲートの緩和」）。
