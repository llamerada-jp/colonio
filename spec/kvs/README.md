...
- [ ] [**is_stable ゲートの緩和**](#todo-is-stable)（設計 + Go） —
      構造的な脆弱性（不安定時の修復凍結）は残るが、根本原因だった seed
      黒穴を 2026-07-11 に別件で解消済みのため、run 15 では「停滞への
      寄与は観測されず」（2026-07-25 現状確認）。優先度は保留に格下げ
...
- **現状確認 (2026-07-25)**: 上記の観測系譜（run 4/11/14）を引き起こしていた
  根本原因は seed セッション喪失による「接続黒穴」ノードで、これは
  2026-07-11 に別件（[spec/seed/README.md](../seed/README.md) 参照）で
  解消済み。run 15（6.8 時間、黒穴 0 体）の実測では
  **is_stable フラップ自体は寿命が延びた分そのまま増加した（median
  13→15 回/寿命、max 47→167）が、「停滞への寄与は観測されず」**
  （README「シミュレーション実行の記録」run 15 節参照）。つまり
  ゲートの構造的な脆弱性（不安定時に修復が完全停止する設計）自体は
  未修正のまま残っているが、それを長時間トリガーしていた根本原因が
  消えたため、現行のシミュレーション条件下では実害が観測できていない。
  対策候補 (a)(b)(c) の実装は見送り、優先度を保留に格下げする。
  黒穴以外の要因（seed 再起動、リンク層の断続的な不安定化等）で
  is_stable フラップが長時間化する経路が将来観測されたら再着手する。
